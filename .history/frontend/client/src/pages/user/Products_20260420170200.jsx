...
const Products = () => {
  ...
  useEffect(() => {
    ...
    if (appliedFilters.category !== 'All') {
      const allowedCategories = categoryMapping[appliedFilters.category] || [appliedFilters.category];
      result = result.filter(p => allowedCategories.includes(p.category));
    }

    result = result.filter(p => p.price <= appliedFilters.priceRange);

    if (appliedFilters.sizes.length > 0) {
      result = result.filter(p => p.sizes && p.sizes.some(s => appliedFilters.sizes.includes(s)));
    }

    if (appliedFilters.brands.length > 0) {
      result = result.filter(p => appliedFilters.brands.includes(p.vendor));
    }

    if (appliedFilters.availability.length > 0) {
      result = result.filter(p => {
        if (appliedFilters.availability.includes('inStock') && p.inStock) return true;
        if (appliedFilters.availability.includes('outStock') && !p.inStock) return true;
        return false;
      });
    }
    ...
    >
      ...
      <div className={`products-page-container ${isLoaded ? 'fade-in' : ''}`}>
        ...
        <aside ref={sidebarRef} className={`product-sidebar ${isFilterOpen ? 'mobile-open' : ''}`}>
          ...
          <div className="sidebar-section apply-filter-section">
